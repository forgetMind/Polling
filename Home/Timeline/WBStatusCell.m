...
@implementation WBVoteEntyView

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    @weakify(self);
    _voteButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [_voteButton setBackgroundImage:[UIImage imageWithColor:kWBCellBackgroundColor] forState:UIControlStateNormal];
    [_voteButton setBackgroundImage:[UIImage imageWithColor:[UIColor colorWithWhite:0.000 alpha:0.200]] forState:UIControlStateHighlighted];
    _voteButton.frame = frame;
    [_voteButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
        if ([weak_self.cell.delegate respondsToSelector:@selector(cellDidClickTag:)]) {
            [weak_self.cell.delegate cellDidClickTag:weak_self.cell];
        }
    }];
...
//    _descriptionLabel.textVerticalAlignment = YYTextVerticalAlignmentCenter;
//    _descriptionLabel.displaysAsynchronously = YES;
//    _descriptionLabel.ignoreCommonProperties = YES;
//    _descriptionLabel.fadeOnHighlight = NO;
//    _descriptionLabel.fadeOnAsynchronouslyDisplay = NO;
//    [self addSubview:_descriptionLabel];
    
    _ratioLabel = [YYLabel new];
    _ratioLabel.frame = CGRectMake(self.width - _ratioLabel.width, 0, _ratioLabel.width, kWBVoteEntyFontSize);
    _ratioLabel.textVerticalAlignment = YYTextVerticalAlignmentCenter;
    _ratioLabel.displaysAsynchronously = YES;
    _ratioLabel.ignoreCommonProperties = YES;
    _ratioLabel.fadeOnHighlight = NO;
    _ratioLabel.fadeOnAsynchronouslyDisplay = NO;
...
- (void)setLayout:(WBStatusLayout *)layout {
    ...
    if (layout.retweetHeight > 0)
    {
        ...
        top += _retweetTextLabel.bottom;
        
        //如果是转发的,并且包含图片
        if (layout.retweetPicHeight > 0)
        {
            [self _setImageViewWithTop:_retweetTextLabel.bottom isRetweet:YES];
            //投票的视图在文字介绍和图片之下，tag之上
            //顶部变成了文字描述加上转发文字描述，加上照片高度
            top += layout.picHeight;
            [self _setVoteViewWithTop:top isRetweet:YES];
        }
        //如果是转发的,不包含图片
        else
        {
            [self _hideImageViews];
            //投票视图
            if (layout.retweetVoteHeight > 0)
            {
                //顶部变成了文字描述加上转发文字描述，
                [self _setVoteViewWithTop: top isRetweet:NO];
...
- (void)_setVoteViewWithTop:(CGFloat)voteTop isRetweet:(BOOL)isRetweet
{
    
    //判断是否是转发的，如果是，则设置成转发的尺寸
    CGSize voteSize = isRetweet ? _layout.retweetVotesSize : _layout.votesSize;
    NSArray *votes = isRetweet ? _layout.status.retweetedStatus.votes : _layout.status.votes;
    int votesCount = (int)votes.count;
    
    for (int i = 0; i < votesCount; ++i)
    {
        WBVoteEntyView * voteEntyView = [[WBVoteEntyView alloc]init];
        CGPoint origin = {0};
        origin.x = kWBCellPadding ;
        origin.y = voteTop + (voteSize.height + kWBCellVoteEntyPadding);
        voteEntyView.frame = (CGRect){.origin = origin, .size = voteSize};
    }
